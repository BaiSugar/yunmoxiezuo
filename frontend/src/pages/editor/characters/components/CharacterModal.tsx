  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (character) {
        ...
        success("更新成功", `人物卡"${name}"已更新`);
      } else {
        // 创建
        await charactersApi.createCharacter(novelId, {
          name: name.trim(),
          category: category.trim() || undefined,
          fields,
        });
        success("创建成功", `人物卡"${name}"已创建`);
      }
      onSave();
    } catch (err: any) {
      console.error("保存人物卡失败:", err);
      showError("保存失败", err.response?.data?.message || "无法保存人物卡");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200">
        ...
        >
          ...
          <div>
            ...
            {Object.keys(fields).length > 0 && (
              <div className="space-y-2 mb-4">
                {Object.entries(fields).map(([key, value]) => (
                  <div key={key} className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-3 border border-blue-100">
                    <div className="flex items-start gap-3">
                      <div className="flex-1">
                        ...
                        <textarea
                          ...
                          className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm resize-none overflow-y-auto max-h-[144px]"
                        />
                      </div>
                    </div>
                  </div>
            ...
            {/* 添加新字段 - 简洁布局 */}
            <div className="bg-white border-2 border-dashed border-gray-300 rounded-lg p-4 hover:border-blue-400 transition-colors">
              <div className="space-y-3">
                <input
                  type="text"
                  value={newFieldKey}
                  onChange={(e) => setNewFieldKey(e.target.value)}
                  placeholder="字段名（如：性别、年龄、职业）"
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm resize-none overflow-y-auto max-h-[144px]"
                />
                <button
                  type="button"
                  onClick={handleAddField}
                  className="w-full py-2 bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white rounded-lg transition-colors flex items-center justify-center gap-2 font-medium"
                >
